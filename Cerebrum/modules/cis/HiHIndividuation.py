...
"""
Functionality for the Individuation project that is specific to HiH.
"""

import cereconf, cerebrum_path
from Cerebrum import Errors
from Cerebrum.Utils import Factory
from Cerebrum.modules.no.hih import PasswordChecker

from Cerebrum.modules.cis import Individuation

class Individuation(Individuation.Individuation):
    """HiH specific behaviour for the individuation service."""
    ...
    # The from address
    email_from = 'noreply@hih.no'

    # The feedback messages for HiH
    messages = {
        ...
        'person_notfound':   {'en': u'Could not find a person by given data, please try again. Please note that you will ' + 
                                    u'not be able to use this service if you are reserved from being published on HiH\'s ' +
                                    u'web pages.',
                              'no': (u'Kunne ikke finne personen ut fra oppgitte data, vennligst prøv igjen. Merk at du ' + 
                                     u'ikke kan bruke denne tjenesten om du har reservert deg fra å bli publisert på HiH ' +
                                     u'sine nettsider.')},
    ...
    def _check_password(self, password, account=None):
        ...
        pc = PasswordChecker.HiHPasswordChecker(db)
        if password:
