...
class UserSync(ADUserUtils):
    ...
    def fullsync(self):
        ...
        # comparison with AD
        for acc in six.itervalues(self.accounts):
        ...
        for uname, ad_user in six.iteritems(addump):
    ...
    def fetch_cerebrum_data(self):
        ...
        if self.exchange_sync:
            ...
            self.logger.info("Fetched %i cerebrum users with both %s and %s"
                             " spreads",
                             len([1 for a in six.itervalues(self.accounts)
        ...
        self.logger.info("Found %i quarantined users",
                         len([1 for v in six.itervalues(self.accounts)
                              if v.quarantined]))
    ...
    def fetch_email_info(self):
        ...
        for uname, prim_mail in six.iteritems(self.ac.getdict_uname2mailaddr(
                filter_expired=True,
                primary_only=True)):
        ...
        if self.exchange_sync:
            # Get all email addrs
            for uname, all_mail in six.iteritems(self.ac.getdict_uname2mailaddr(
                    filter_expired=True,
                    primary_only=False)):
                acc = self.accounts.get(uname)
                if acc:
    ...
    def fetch_ad_data_contacts(self):
        ...
        if ad_contacts:
            ...
            for object_name, properties in six.iteritems(ad_contacts):
                # TBD: cereconf-var?
    ...
    def fetch_ad_data_distribution_groups(self):
        ...
        if ad_dist_grps:
            ...
            # groups.
            for grp_name, properties in six.iteritems(ad_dist_grps):
    ...
    def compare(self, ad_user, cb_user):
        ...
        for attr, value in six.iteritems(cereconf.AD_ACCOUNT_CONTROL):
    ...
    def compare_forwards(self, ad_contacts):
        ...
        for acc in six.itervalues(self.accounts):
            for contact in acc.contact_objects:
                ...
                #   for attr_type, cb_fwd_attr in six.iteritems(fwd):
            ...
            for ad_fwd in six.itervalues(ad_contacts):
                self.delete_contact()
...
class GroupSync(ADGroupUtils):
    ...
    def fullsync(self):
        ...
        for gname, ad_group in six.iteritems(addump):
        ...
        for grp in six.itervalues(self.groups):
    ...
    def fetch_cerebrum_data(self):
        ...
        for g in six.itervalues(self.groups):
            g.calc_ad_attrs()
