...
class _ConsentTypeCode(Cerebrum.Constants._CerebrumCode):
    """
    ...
    _insert_dependency = Cerebrum.Constants._EntityTypeCode


class _EntityConsentCode(Cerebrum.Constants._CerebrumCodeWithEntityType):
    """
        ...
        self.sql.execute(
            """
              INSERT INTO %(code_table)s
                (consent_type, entity_type, %(code_col)s, %(str_col)s,
                %(desc_col)s)
              VALUES
    ...
    def consent_type(self):
        ...
        if not hasattr(self, '_consent_type'):
            self._consent_type = _ConsentTypeCode(
                self.sql.query_1(
                    """
                      SELECT consent_type
                      FROM %(table)s
...
class Constants(Cerebrum.Constants.Constants):
    ...
    consent_opt_in = _ConsentTypeCode(
        'opt-in',
        'Consent actively given',
    )

    consent_opt_out = _ConsentTypeCode(
        'opt-out',
...
class CLConstants(Cerebrum.Constants.CLConstants):
    ...
    consent_approve = Cerebrum.Constants._ChangeTypeCode(
        'consent',
        'approve',
    ...
    consent_decline = Cerebrum.Constants._ChangeTypeCode(
        'consent',
        'decline',
    ...
    consent_remove = Cerebrum.Constants._ChangeTypeCode(
        'consent',
        'delete',
