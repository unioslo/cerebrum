...
class DnsBofhdUtils(IPUtils, DnsParser):
    ...
    def alter_ttl_record(self, owner_id, ttl_type, ttl, dta):
        self._dns_owner.clear()
    ...
    def alter_srv_record(self, operation, service_name, ttl, pri,
                         weight, port, target):
    ...
    def mx_set_add(self, mx_set, ttl, priority, target_id):
        if ttl:
            ttl = int(ttl)
        else:
            ttl = None
