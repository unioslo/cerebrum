...
class LdapCall:
    """ Only used in generate_posix_ldif, for now... """
    ...
    def end_session(self):
        for serv, value in self.s_list.items():
            try:
                ...
                logger.warn("Could not close LDAP/SSL to server: %r", serv)
            value[0].write("\n# Closed TLS-connection and log_file.")
            logger.info('Closed TLS-chanel and log to server %r', serv)
            try:
                value[0].close()
    ...
    def get_ldap_value(self, search_id, dn, retrieveAttributes=None):
        ...
        for serv, l in self.s_list.items():
            try:
                ldap_result_id = l[1].search(search_id, searchScope, dn,
                                             retrieveAttributes)
                while 1:
    ...
    def mod_ldap_serv(self, ldap_mod, attr, attr_value, dn_value, k,
                      list=None):
        if list:
            ldif_list = [(ldap_mod, attr, attr_value)]
        else:
