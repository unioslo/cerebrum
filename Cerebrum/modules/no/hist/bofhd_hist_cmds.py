...
class BofhdExtension(object):
    ...
    def _email_info_detail(self, acc, addrs):
        ...
        local_copy = ""
        ef = Email.EmailForward(self.db)
        ef.find_by_target_entity(acc.entity_id)
        for r in ef.get_forward():
            if r['enable'] == 'T':
                enabled = "on"
            else:
	...
	return "Request sent to it-tjenesten@ahs.hist.no"
...
#            raise CerebrumError, "Cannot remove IMAP spread without deleting user"
	...
	if (self._get_constant(spread, "No such spread") not in \
	    [self.const.spread_HiST_nds_stud_aft, self.const.spread_HiST_nds_stud_aoa,
	     self.const.spread_HiST_AD_stud]):
    ...
    def _find_persons(self, arg):
        ...
        if arg.find(":") != -1:
            ...
            if idtype == 'exp':
                ...
                for ss in [self.const.system_fs,
