...
####
    ...
    def remove_address(self, event):
        ...
        if eit == self.co.entity_account:
            ...
            uname = self.ut.get_account_name(eid)
            try:
                self.ec.remove_mailbox_addresses(uname, [address])
    ...
    def create_group(self, event):
        ...
        if added_spread_code == self.group_spread:
            ...
            try:
                ...
                self.logger.warn('eid:%d: Can\'t find group %d',
...
# TODO: This won't really work. Not implemented. Fix it somehow
                    ...
                    ev_mod = event.copy()
                    (ev_mod['subject_entity'],
                     tra, sh, hq, sq) = self.ut.get_email_target_info(
                        target_entity=event['subject_entity'])
                ...
                try:
                    ...
                    self.logger.info(
                        'eid:%d: Set addresses for %s: %s',
                        event['event_id'], gname, str(data['aliases']))
            ...
            if data['roomlist'] == 'F':
                ...
                if data['modby']:
                    try:
                        self.ec.set_distgroup_moderator(
                            gname, ', '.join(data['modby']))
                        self.logger.info(
                            'eid:%d: Set moderators %s on %s',
...
# TODO: Receipt for this?
                        ...
                        self.logger.warn(
                            ...
                            event['event_id'], gname, data['modenable'], str(e))
                        ev_mod = event.copy()
                        ev_mod['change_params'] = pickle.dumps(
        ...
        if self.group_spread in group_spreads:
            ...
            try:
                ...
                self.logger.warn(
                    ...
                    event['event_id'], attrs['name'], attrs['description'], e)
                raise EventExecutionException
