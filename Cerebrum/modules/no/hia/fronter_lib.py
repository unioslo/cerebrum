...
class Fronter(object):
	...
	#Må byttes ut med XML-parser mot underv_enhet (current og next)
...
class hiafronter(Person.Person):

    __read_attr__ = ('__in_db', '_extid_source', '_extid_types')

    def clear(self):
        self.__super.clear()
        self.clear_class(hiafronter)
        self.__updated = []
    # end clear
	...
	return self.query("""
	  ...
	  JOIN [:table schema=cerebrum name=entity_name] en
          ...
          JOIN [:table schema=cerebrum name=person_external_id] pei
            ON pi.person_id = pei.person_id AND
	  ...
	  LEFT JOIN [:table schema=cerebrum name=email_target] et
		...
		et.target_type = [:get_constant name=email_target_account] AND
		et.entity_type = [:get_constant name=entity_account]
