...
def format_cl_role_type(co, val):
    return _get_code(co.ElementsRole, val)


@Constants._ChangeTypeCode.formatter('perm_type')
def format_cl_perm_type(co, val):
    return _get_code(co.ElementsPermission, val)
...
#
#        * hvis en person slettes skal dens elements-roller,
...
class ElementsRole(DatabaseAccessor):
    ...
    def _add_role(self, person_id, role, sko, arkivdel, journalenhet,
        ...
        self.execute("""
        INSERT INTO [:table schema=cerebrum name=elements_role]
          (%s) VALUES (%s)""" % (", ".join(binds.keys()),
                                 ", ".join([":%s" % k for k in binds])),
                     binds)
        self._db.log_change(
            person_id, self.clconst.elements_role_add, sko,
            change_params={
    ...
    def set_standard_role_val(self, person_id, role, sko, arkivdel,
        ...
        query = """
        UPDATE [:table schema=cerebrum name=elements_role]
        SET standard_role='%s'
        WHERE %s""" % (standard_role, " AND ".join(
            ["%s=:%s" % (x, x) for x in binds if binds[x]]))
        self.execute(query, binds)
        self._db.log_change(
            person_id, self.clconst.elements_role_upd, sko,
    ...
    def _remove_role(self, person_id, role, sko, arkivdel, journalenhet):
        ...
        exists_stmt = """
           SELECT EXISTS (
            SELECT 1
            FROM [:table schema=cerebrum name=elements_role]
            WHERE %s
          )
        """ % where_str
        if not self.query_1(exists_stmt, binds):
            ...
            return
        delete_stmt = """
        DELETE FROM [:table schema=cerebrum name=elements_role]
        WHERE %s""" % where_str
        self.execute(delete_stmt, binds)
        self._db.log_change(
            person_id, self.clconst.elements_role_rem, sko,
            change_params={
                'arkivdel': arkivdel and str(arkivdel) or '',
                'rolle_type': str(role)
            })
    ...
    def remove_role(self, person_id, role, sko, arkivdel, journalenhet):
        ...
        # If person doesn't have any roles left, any elements
        # permissions and the elements-spread should be removed
        if not self.list_roles(person_id=person_id):
            # This is ugly :(
            # Remove eny permissions before deleting elements spread
            for row in self.elements_perm.list_permission(person_id=person_id):
                self.elements_perm.remove_permission(row["person_id"],
                                                    row["perm_type"],
                                                    row["adm_enhet"])
            # Then remove spread
            self.pe.clear()
            self.pe.find(person_id)
            self.pe.delete_spread(self.co.spread_elements_person)
    ...
    def list_roles(self, person_id=None, filter_expired=False):
        ...
        SELECT person_id, role_type, standard_role, adm_enhet,
               arkivdel, journalenhet, rolletittel, stilling,
               start_date, end_date, auto_role
        FROM [:table schema=cerebrum name=elements_role] %s
...
class ElementsPermission(DatabaseAccessor):
    def __init__(self, database):
        super(ElementsPermission, self).__init__(database)
        self.clconst = Factory.get('CLConstants')(database)
        self.pe = Factory.get('Person')(database)
    ...
    def add_permission(self, person_id, perm_type, sko, requestee):
        ...
        self.execute("""
        INSERT INTO [:table schema=cerebrum name=elements_permission]
          (%s) VALUES (%s)""" % (", ".join(binds.keys()),
                                 ", ".join([":%s" % k for k in binds])),
                     binds)
        self._db.log_change(
            person_id, self.clconst.elements_perm_add, sko,
            change_params={
                'adm_enhet': sko or '',
    ...
    def remove_permission(self, person_id, perm_type, sko):
        ...
        exists_stmt = """
          SELECT EXISTS (
            SELECT 1
            FROM [:table schema=cerebrum name=elements_permission]
            WHERE %s
          )
        """ % where
        if not self.query_1(exists_stmt, binds):
            ...
            return
        delete_stmt = """
        DELETE FROM [:table schema=cerebrum name=elements_permission]
        WHERE %s""" % where
        self.execute(delete_stmt, binds)
        self._db.log_change(
            person_id, self.clconst.elements_perm_rem, sko,
            change_params={
                'adm_enhet': sko or '',
                'perm_type': str(perm_type)
            })
    ...
    def expire_permission(self, person_id, perm_type, sko, end_date=None):
        ...
        query = """
        UPDATE[:table schema=cerebrum name=elements_permission]
        SET end_date=%s
        WHERE %s""" % (
    ...
    def list_permission(self, person_id=None, perm_type=None, adm_enhet=None,
        ...
        FROM [:table schema=cerebrum name=elements_permission] %s""" % where, {
            'person_id': person_id,
            'perm_type': perm_type,
            'adm_enhet': adm_enhet})
