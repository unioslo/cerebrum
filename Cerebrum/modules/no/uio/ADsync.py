...
class ADFullUserSync(ADutilMixIn.ADuserUtil):
    ...
    def fetch_cerebrum_data(self, spread):
        ...
        for k in sorted(bydfg, key=lambda x: len(bydfg[x]), reverse=True):
            ...
            if max < 1:
                break
        self.logger.debug(
            "Number of users without DFG: %d", len(bydfg.get(None, ())))
        max = 10
    ...
    def fetch_ad_data(self, search_ou):
        ...
        # part of the SID.
        self.logger.debug(
            "Fetching group SIDs for posix, OU: %s",
            self.ad_ldap)
            ...
            if cerebrumusrs.has_key(usr):
                ...
                for attr in cereconf.AD_ATTRIBUTES:
                    ...
                    if attr == 'homeDrive':
                        ...
                        # self.logger.debug("primaryGroupID: %s",
                        # cerebrumusrs[usr])
    ...
    def get_pwds_from_cl(self, adusrs, dry_run, spread, commit_changes=False):
        ...
        for ans in answer:
            ...
            if ans['change_type_id'] == self.co.account_password and not ans['subject_entity'] in handled:
                ...
                self.logger.debug(
                    "unknown change_type_id %i or user already updated",
                    ans['change_type_id'])
            if confirm:
...
class ADFullGroupSync(ADutilMixIn.ADgroupUtil):
    ...
    def fetch_cerebrum_data(self, spread):
        ...
        for row in self.pu.list_posix_users(filter_expired=True):
            groupid2uids.setdefault(
                row['gid'],
                []).append(str(row['posix_uid']))
        i = 0
            ...
            if add_users:
                ...
                for usr in self.group.search_members(group_id=gruppe_id,
                    ...
                    if primary:
                        ...
                        self.logger.debug(
                            "Person %s has no primary account" %
                            usr['member_id'])
            ...
            if cerebrum_dict[grp].has_key('grp_id'):
                ...
                if members_add or members_remove:
                    ...
                    if not dn:
                        ...
                        self.logger.debug(
                            "Dryrun: don't sync members for %s",
                            grp)
                    else:
                        self.server.bindObject(dn)
                        if members_add:
                            self.logger.info("Adding members to group %s (%s)",
                                             grp, members_add)
                            res = self.server.addMembers(
                                members_add, sendDN_boost)
                            if not res[0]:
                                self.logger.warning(
    ...
    def full_sync(self, delete=False, dry_run=True, store_sid=False,
        ...
        self.logger.info(
            "Comparing %i ad-groups after filtering" %
            len(addump))
        ...
        if full_membersync:
            ...
            self.compare_members(
                cerebrumdump, addump, group_spread, user_spread,
                dry_run, sendDN_boost)
