...
class ADFullUserSync(ADutils.ADuserUtil):
    ...
    def _update_home_drive_info(self, user_dict, spread):
        ...
        for k, v in six.iteritems(user_dict):
            if k in uname2disk:
    ...
    def fetch_cerebrum_data(self, spread):
        ...
        self.logger.debug("%i accounts with spread %r after quarantine filter",
                          len(tmp_ret), spread)
        a = 0
        for u in six.itervalues(tmp_ret):
            if u['ACCOUNTDISABLE']:
                a += 1
        ...
        i = 0
        for k, v in six.iteritems(tmp_ret):
            if k in posixusers:
        ...
        #
        for k, v in six.iteritems(userdict_ret):
    ...
    def compare(self, delete_users, cerebrumusrs, adusrs):
        ...
        for usr, dta in six.iteritems(adusrs):
    ...
    def fetch_cerebrum_data(self, spread):
        ...
        for row in self.pu.list_posix_users(filter_expired=True):
            groupid2uids.setdefault(row['gid'], []).append(
                str(row['posix_uid']))
        i = 0
        for gname, gdata in six.iteritems(grp_dict):
        ...
        i = 0
        for gdata in six.itervalues(grp_dict):
