...
class ADObject(object):
    ...
    def bindObject(self, LDAPAccount):
        try:
            self.Object=win32com.client.GetObject('LDAP://%s' % LDAPAccount)
        ...
        logging.debug("Object bound to %s" % self.distinguishedName)
    ...
    def deleteObject(self):
        ...
        self.clearObject()          
        return (True,'deleteObject %s %s,%s' % (objtype, name,OU))
                ...
                try:
                    if remove:
                        if self.Object.isMember('LDAP://%s' % memb):
                            ...
                            logging.debug('addremovemembers User %s removed from %s' % (
				memb, self.distinguishedName))          
                        else:   
                            logging.debug('addremoveMembers: User %s not member of %s' % (
				memb, self.distinguishedName))
                    else:
                        if not self.Object.isMember('LDAP://%s' % memb):
                            self.Object.Add('LDAP://%s' % memb)
                            logging.debug('addremovemembers User %s added to %s' % (
				memb, self.distinguishedName))      
                        else:
                            logging.debug('addremovemembers User %s already member of %s' % (
				memb, self.distinguishedName))     
                except pythoncom.com_error:
                ...
                for memb in memberlist:
                    ...
                    if tmpmemb:
                        ...
                        if reportmissing: 
                            ...
                            error = True
                        logging.warn('syncMembers User %s to %s do not exist' % (
			    memb, self.distinguishedName))
            else:
                cerememb = memberlist   
                ...
                if mem in admemb:
                    ...
                    if not self.Object.isMember('LDAP://%s' % mem):
                        try:
                            ...
                            logging.debug('syncMember User %s Added to %s' % (
				mem, self.distinguishedName))     
                        except pythoncom.com_error:
                            logging.warn("syncMember Add %s to %s failed: %s" % (
				mem, self.distinguishedName, sys.exc_info()[1]))
                            errors = '%s%s' % (errors, mem)
                            error = True
                    else:
                        logging.debug('syncMember User %s already member of %s' % (
			    mem, self.distinguishedName))        
            ...
            for membrs in admemb:       
                if self.Object.isMember('LDAP://%s' % membrs):
                    try:
                        ...
                        logging.debug('syncMember User %s removed from %s' % (
			    membrs, self.distinguishedName))      
                    except pythoncom.com_error:
                        logging.warn("syncMember %s failed Remove %s: %s" % (
			    self.distinguishedName, membrs, sys.exc_info()[1]))
                        errors = '%s%s' % (errors, membrs)
                        error = True
                else:
                    logging.debug('syncMember User %s not a member of %s' % (
			membrs, self.distinguishedName))           
...
class Search(ADObject):
    ...
    def listObjects(self, type, prop = False, OU = const.AD_LDAP_ROOT):
        ...
        self.objCom.CommandText = ("SELECT '%s' FROM 'LDAP://%s' where %s" % (
	    fields, OU, objecttype))
	logging.debug("listObjects cmd: %s" % self.objCom.CommandText)
    ...
    def findObject(self, account, OU=False):
        ...
        try:
            ...
            logging.warn('findObject on %s (%s)' % (account, exc))
            return False
