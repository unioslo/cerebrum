...
def full_sync(delete_groups, dry_run):
	...
	#Fetch cerebrum data.
	print "group_spread:%i" % int(co.spread_uio_ad_group)
	cerebrumgroups = fetch_cerebrum_data(int(co.spread_uio_ad_group))
...
def sync_groups(cerebrumgroups, dry_run):
	...
	for (grp_id, grp_name, grp_desc) in cerebrumgroups:
		...
		group.clear()
		group.find(grp_id)				 
		user_memb = group.list_members(spread=co.spread_uio_ad_account, 
										get_entity_name=True)
		group_memb = group.list_members(spread=co.spread_uio_ad_group, 
										get_entity_name=True)
		members = []
		...
		if not dn:
			logger.debug("unknown group: %s%s" % (grp_name, 
												cereconf.AD_GROUP_POSTFIX))	
		else:
			server.bindObject(dn)
			res = server.syncMembers(members, False, False)
			if not res[0]:
...
def compare(delete_groups,cerebrumgrp,adgrp):
	...
	for (grp_id, grp, description) in cerebrumgrp:
		if 'CN=%s%s,%s,%s' % (grp, cereconf.AD_GROUP_POSTFIX,
							cereconf.AD_DEFAULT_GROUP_OU,
							cereconf.AD_LDAP) in adgrp:
			adgrp.remove('CN=%s%s,%s,%s' % (grp, cereconf.AD_GROUP_POSTFIX,
							cereconf.AD_DEFAULT_GROUP_OU,
							cereconf.AD_LDAP))
		else:
			#Group not in AD, create.
			changelist.append({'type': 'createObject',
				'sAMAccountName' : '%s%s' % (grp, cereconf.AD_GROUP_POSTFIX),
