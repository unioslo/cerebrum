...
This file contains code which import historical account data into
...
def process_line(infile):
    ...
    for line in stream:
        ...
        gname, members = fields

        # Check group
        group_id = process_group(gname.strip())
        if group_id:
            # Group exists, check members
            for uname in [u.strip() for u in members.split(',')]:
                account_id = process_account(group_id, uname)
        else:
...
def process_group(gname):
    ...
    try:
        ...
        logger.debug("Found group %s", gname)
