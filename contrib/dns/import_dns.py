...
class FileParsers(object):
    def parse_hosts_file(self, fname):
        ...
        while 1:
            ...
            rest = rest.strip()
            if not hostname[-1] == '.':
                hostname = hostname + zone.postfix
            ret[hostname] = rest
        return ret
...
def filter_name(name):
    if not name.endswith(zone.postfix):
        if name[-1] == '.':
...
def _make_foreign_owner(name):
    return _make_dns_owner(co.other_zone, name)

def _make_dns_owner(zone, name):
    dnsowner.clear()
...
def pass_two(name, dta):
    ...
    if dta.has_key('CNAME'):
        ...
        cname.clear()
        cname.populate(_make_dns_owner(zone, name)[1], target_owner_ids[1], cname_ttl)
        cname.write_db()
