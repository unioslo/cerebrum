...
class ZoneUtils(object):
    ...
    def write_file_with_serial(self, dta_file, fname, heads):
      ...
      serial_file, serial = self.__write_heads(f, heads)
    ...
    def __write_heads(self, f, heads):
...
class ReverseMap(object):
    def __init__(self, mask):
        self.zu = ZoneUtils(None)
        self.ip_numbers = {}
        self.a_records = {}
        self.override_ip = {}
        ipu = IPCalc()
        mask = mask.split("/")
        self._get_reverse_data(*ipu.ip_range_by_netmask(mask[0], int(mask[1])))
    ...
    def generate_hosts_file(self, fname):
        ...
        for a_id in order:
            ...
            line = '%s\t%s %s' % (a_ref['a_ip'], a_ref['name'],
                                  self.zu.exp_name(a_ref['name'], no_dot=True))
            ...
            line += " ".join(
                ["%s %s" % (n, self.zu.exp_name(n, no_dot=True)) for n in names])
            f.write(line+"\n")
        f.close()
