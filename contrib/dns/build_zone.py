...
## >>> o = psycopg.connect('dbname=cerebrum user=cerebrum')
## >>> c = o.cursor()
## >>> c.execute("SELECT hinfo FROM dns_host_info WHERE 0=1")
## >>> oid = c.description[0][1]
## >>> oid
## 1700
## >>> MY_INT = psycopg.new_type((oid,), "MY_INT", lambda x: int(x))
...
def generate_reverse_file(fname):
    f = open(fname, 'w')
    #f = sys.stdout
    f.write(header_splitter)
...
def usage(exitcode=0):
    ...
    ./contrib/strip4cmp.py uio.no-gen > uio.no-gen.strip
    sort -u uio.no-gen.strip > uio.no-gen.strip.sort
    diff -u uio.no-gen.strip.sort uio.no.strip.sort > feil.diff
    
    TODO: Add support for building reverse-map
    """
    sys.exit(exitcode)
...
def main():
    ...
    for opt, val in opts:
        if opt in ('--help', '-h'):
            ...
            generate_zone_file(val)
        elif opt in ('--reverse', '-r'):
            generate_reverse_file(val)
        elif opt in ('--hosts', ):
