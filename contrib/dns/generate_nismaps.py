...
class NISGroupUtil(object):
    ...
    def _expand_group(self, gid, flatten=False):
        ...
        for row in u:
            uname = self._entity2name.get(int(row[1]), None)
            if not uname:
                logger.warn("Was %i very recently created?" % int(row[1]))
                continue
            ret_non_groups.append(uname)
        ...
        for row in u:
            ...
            if self._exported_groups.has_key(gid):
                ...
                ret_groups.append(t_g)
                ret_non_groups.append(t_ng)
    ...
    def generate_netgroup(self, filename):
        ...
        for group_id in self._exported_groups.keys():
            ...
            line = " ".join((" ".join(group_members),
                             " ".join(["(%s,-,)" % m[:-len_zone] for m in user_members
                                       if m.endswith(zone)]),
                             " ".join(["(%s,-,)" % m[:-1] for m in user_members]),
                             ))
            f.write(self._wrap_line(self._exported_groups[group_id], line))
        f.close()
...
def main():
    ngu = NISGroupUtil(co.dns_owner_namespace, co.entity_dns_owner,
                       _SpreadCode('NIS_mng@uio'), None)
    ngu.generate_netgroup('test.dump')
