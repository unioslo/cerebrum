...
def user2URL(user):
    ...
    return "http://folk.uio.no/{}/".format(user)
...
def main():
    ...
    person2URLs = defaultdict(list)    # {person_id: [desired URL, ...]}
    for row in account.list_accounts_by_type():
        ...
        if user in home_page_users:
            ...
            person2URLs[int(row['person_id'])].append(user2URL(user))
...
Use the '--force' argument to force this change."""
    ...
    for person_id, new in person2URLs.items():
        person.clear()
