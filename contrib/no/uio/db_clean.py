...
from Cerebrum.modules.ConfigData import CLConfigData
...
* check the class CLConfigData for the parameter values regarding the ChangeLog cleaning.
...
def format_as_int(i):
    ...
    if i is not None:
        return int(i)
    return i
CLConfigDataInstance = CLConfigData()
# Here could be forced the instance specific configuration data variabels like
# for example: CLConfigDataInstance.minimum_age = 3600*24*6 which would override
# the default value which is a little bit different (
# CLConfigDataInstance.minimum_age = 3600*24*6 + 3600*22)
class CleanChangeLog(object):
    def process_log(self):
        ...
        for e in db2.get_log_events():
            ...
            if age < CLConfigDataInstance.minimum_age:
                continue

            tmp = CLConfigDataInstance.max_ages.get(change_type, CLConfigDataInstance.default_age)
            if tmp != CLConfigDataInstance.AGE_FOREVER and age > tmp:
    ...
    def _setup(self):
        ...
        i = 0
        for k in CLConfigDataInstance.keep_togglers:
            k['toggler_id'] = i
            i += 1
            for t in k['triggers']:
