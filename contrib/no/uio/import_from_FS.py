...
default_topics_file = "topics.xml"
default_regkort_file = "regkort.xml"
default_betalt_papir_file = "betalt_papir.xml"
default_edu_file = "edu_info.xml"
default_pre_course_file = "pre_course.xml"
...
def usage():
    ...
    --pre-course-file: Name of output file for pre course information. Default: pre_course.xml.
    --regkort-file: Override regkort xml filename. Default: regkort.xml.
    --betalt-papir-file: Override betalt-papir xml filename. Default: betalt_papir.xml.
...
def write_person_info(outfile):
    ...
    for s in students:
        ...
        f.write(
            xml.xmlify_dbrow(
                s, xml.conv_colnames(cols), 'privatist_studieprogram') + "\n")
    ...
    for s in students:
        f.write(
            xml.xmlify_dbrow(s, xml.conv_colnames(cols), 'emnestud') + "\n")
    ...
    for p in permstud:
        f.write(
            xml.xmlify_dbrow(p, xml.conv_colnames(cols), 'permisjon') + "\n")

    #
    # STA har bestemt at personer med tilbud ikke skal ha tilgang til noen
    # IT-tjenester inntil videre. Derfor slutter vi på nåværende tidspunkt å
    # hente ut informasjon om disse. Ettersom det er usikkert om dette vil
    # endre seg igjen i nær fremtid lar vi koden ligge for nå.
    #
...
def main():
    ...
    edu_file = default_edu_file
    pre_course_file = default_pre_course_file
    ...
    for o, val in opts:
        try:
            if o in ('-p',):
                ...
                write_betalt_papir_info(set_filepath(datadir, betalt_papir_file))
            elif o in ('-R',):
