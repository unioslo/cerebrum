...
def search_full_name(k,v):
    ...
    try:
        ...
        pos_pers = person.search_person_names(name=name,
                                              case_sensitive=True,
                                              exact_match=False)
        for x in pos_pers:
            person.clear()
            person.find(int(x['person_id']))
            fdsl = person.get_external_id(id_type=co.externalid_fodselsnr)
            name_hit_file.write("Account:%s Person: %s fdsl.nr: %s " % (k,name,fdsl[0]['external_id']))
            n = ':'.join((k,v))
            process_account(n, personid=fdsl[0]['person_id'])
            return True
        else:
            return False
    except:
...
# arch-tag: ba7a8519-3ea4-4097-894b-11c771b74bf9
