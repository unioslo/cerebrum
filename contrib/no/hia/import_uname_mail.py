...
def process_line(infile):
    ...
    while l:
        ...
        fld = l.split(':')
        if len(fld) <> 4:
            sys.stderr.write("Bad line: %s. Skipping" % l)
            continue
        ...
        if not fnr == "":
            ...
            if p:
                ...
                sys.stderr.write("Bad fnr: %s Skipping" % l)
                continue
        else:
            if verbose > 1:
        ...
        if a:
            ...
            sys.stderr.write("Bad uname: %s Skipping" % l)
        l = f.readline().strip()
        db.commit()
...
def process_mail(acc_id, type, addr):
    ...
    fld = addr.split('@')
    if len(fld) <> 2:
        sys.stderr.write("Bad address: %s. Skipping" % addr)
        return None
    lp, dom = fld
    try:
        edom.find_by_domain(dom)
        if verbose > 0:
            print "Domain found: %s" % dom
    except Errors.NotFoundError:
        edom.populate(dom, "Generated by import_uname_mail.")
        edom.write_db()
        #db.commit()
        if verbose > 0:
            print "Domain created: %s" % dom            
    try:
        et.find_by_entity(int(acc_id))
        if verbose > 0:
            print "EmailTarget found(account): %s" %acc_id  
    except Errors.NotFoundError:
        ...
        #db.commit()
        if verbose > 0:
            print "EmailTarget created: %s" % acc_id 
    try:
        ea.find_by_address(addr)
        if verbose > 0:
            print "EmailAddress found: %s" % addr 
    except Errors.NotFoundError:
        ea.populate(lp, edom.email_domain_id, et.email_target_id)
        ea.write_db()
        #db.commit()
        if verbose > 0:
            print "EmailAddress created: %s" % addr 
    if type == "defaultmail":
        try:
            epat.find(et.email_target_id)
            if verbose > 0:
                print "EmailPrimary found: %s" % addr 
        except Errors.NotFoundError:
            epat.clear()
            ...
            #db.commit()
            if verbose > 0:
                print "EmailPrimary created: %s" % addr 
    et.clear()
    ea.clear()
    edom.clear()
    epat.clear()
