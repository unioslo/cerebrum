...
#     removed from Cerebrum database.
    ...
    for ans in answer:
		...
		print 'WARNING: failed changing password for ',ans['subject_entity']
            ...
            if entity.has_spread(int(co.spread_hia_ad_account)):
                group.clear()
			...
			if group_rem(account_name,group_name):
			    ...
			    print 'WARNING: failed removing',account_name,' from group ',group_name            
                    ...
                    if debug:
                        print ans['dest_entity'], 'add/rem group: missing spread_hia_ad_group'
            else:
                if debug:
                    print ans['subject_entity'],' add/rem group: missing spread_hia_ad_account'
...
def change_quarantine(entity_id):
    ...
    if account.has_spread(int(co.spread_hia_ad_account)):	
	    ...
	    print "User got not quarantined"
	    add_spread(entity_id,co.spread_hia_ad_account)


def add_spread(entity_id,spread):
    if spread == co.spread_hia_ad_account:
        ...
        try:
            ...
            for aff in affiliations:
                if int(co.affiliation_ansatt) in aff:
                    ad_ou = 'OU=user,OU=ans,%s' % cereconf.AD_LDAP
                elif int(co.affiliation_student) in aff:
                    ad_ou = 'OU=user,OU=stud,%s' % cereconf.AD_LDAP
        except Errors.NotFoundError:
            pass
    ...
    elif spread == co.spread_hia_ad_group:
