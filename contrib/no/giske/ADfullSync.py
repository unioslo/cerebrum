        ...
        for row in self.ac.list_accounts_by_type(account_spread=spread):
            ...
            if row['affiliation'] == co.affiliation_ansatt or row['affiliation'] == co.affiliation_teacher:
                ...
                accinfo[row['account_id']] = {'OU' : 'OU=TILSETTE,%s' % cereconf.AD_LDAP,
                                              ...
                                              'homeMDB' : 'CN=Tilsette_due,CN=First Storage Group,CN=InformationStore,CN=DUE,CN=Servers,CN=Exchange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=Giske grunnskule,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=skule,DC=giske,DC=no'}
                ...
                home_mdb = 'CN=Foresatte_due,CN=First Storage Group,CN=InformationStore,CN=DUE,CN=Servers,CN=Exchange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=Giske grunnskule,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=skule,DC=giske,DC=no'
                # find all person affs in order to build appropriate urls
                self.person.clear()
                try:
                    self.person.find(row['person_id'])
                ...
                for r in self.person.get_affiliations():
                    if r['affiliation'] == co.affiliation_ansatt or \
                        ...
                        home_mdb = 'CN=Tilsette_due,CN=First Storage Group,CN=InformationStore,CN=DUE,CN=Servers,CN=Exchange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=Giske grunnskule,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=skule,DC=giske,DC=no'
                        tils_skole = id2ou[r['ou_id']]['acronym']
                        urls.append('https://portal.skule.giske.no/skule/' + tils_skole + '/tilsette')
                    elif r['affiliation'] == co.affiliation_foresatt:
                        for_skole = id2ou[r['ou_id']]['acronym']
                ...
                if id2ou.has_key(row['ou_id']):
                    accinfo[row['account_id']] = {'OU' : 'OU=ELEVER,OU=%s,%s' %
                                                  ...
                                                  #Constraint i AD, homeMDB must be valid LDAP path.	  
                                                  'homeMDB' : 'CN=Elever_due,CN=First Storage Group,CN=InformationStore,CN=DUE,CN=Servers,CN=Exchange Administrative Group (FYDIBOHF23SPDLT),CN=Administrative Groups,CN=Giske grunnskule,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=skule,DC=giske,DC=no'}
                else:
                    miss_OU = miss_OU + 1
