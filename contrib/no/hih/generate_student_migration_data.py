...
Usage: %s [options]
   ...
   <name>;<no_ssn>;<card-ID>;<uname>

   name    -- Concatenated from first name and last name
   no_ssn  -- 11-digit Norwegian social security number (fødselsnummer)
   card-ID -- ID for Bewator security system (derived from student ID)
...
def main(argv=None):
    ...
    for student_row in student_rows:
        ...
        card_ID = "01221%06d0" % student_row["studentnr_tildelt"]
        uname = "%06d" % student_row["studentnr_tildelt"]
        output_stream.write("%s\n" % ";".join((name, no_ssn, card_ID, uname)))
