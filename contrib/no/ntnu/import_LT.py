...
def get_sted(fakultet, institutt, gruppe):
    ...
    if not ou_cache.has_key(stedkode):
        ...
        try:
            ...
            addr_street = ou.get_entity_address(source=const.system_kjernen,
            ...
            if len(addr_street) > 0:
                ...
                addr_street = None
            addr_post = ou.get_entity_address(source=const.system_kjernen,
            ...
            if len(addr_post) > 0:
                ...
                addr_post = None
            fax = ou.get_contact_info(source=const.system_kjernen,
...
def process_person(person):
    ...
    try:
        ...
        logger.warning("%s has invalid fodselsnr. Reason: %s" % (person,str(e)))
        num_nowrite += 1
        print "%s has invalid fodselsnr. Reason: %s" % (person,str(e))
        return
    ...
    try:
        ...
        try:
            new_person.find_by_external_id(
                const.externalid_fodselsnr, fnr, const.system_kjernen)
        except Errors.NotFoundError:
            pass
    ...
    new_person.affect_names(const.system_kjernen, const.name_first, const.name_last,
						const.name_personal_title)
    new_person.affect_external_id(const.system_kjernen, const.externalid_fodselsnr)
    ...
    new_person.populate_external_id(
        const.system_kjernen, const.externalid_fodselsnr, fnr)
    ...
    new_person.affect_names(const.system_kjernen, const.name_work_title)
    affiliations = determine_affiliations(person)
    new_person.populate_affiliation(const.system_kjernen)
        ...
        if sted is not None:
            if sted['addr_street'] is not None:
                new_person.populate_address(
                    const.system_kjernen, type=const.address_street,
                    **sted['addr_street'])
            if sted['addr_post'] is not None:
                new_person.populate_address(
                    const.system_kjernen, type=const.address_post,
                    **sted['addr_post'])
	...
	ou_id, aff, aff_stat = v
        new_person.populate_affiliation(const.system_kjernen, ou_id,\
...
#               got_fax = True
	if include_del:
		...
		cere_list[k] = False
    c_prefs = {}
    new_person.populate_contact_info(const.system_kjernen)
    for c_type, value in contact:
        c_type = int(c_type)
        pref = c_prefs.get(c_type, 0)
        new_person.populate_contact_info(const.system_kjernen, c_type, value, pref)
    ...
    #for k,v in affiliations.items():
    #ou_id, aff, aff_stat = v
    #    new_person.set_affiliation_last_date(const.system_kjernen, ou_id,\
            #                                     int(aff), int(aff_stat))
...
def load_all_affi_entry():
    affi_list = {}
    for row in new_person.list_affiliations(source_system=const.system_kjernen):
	    ...
	    new_person.entity_id = int(ent_id)
            ...
            for aff in affs:
                ...
                if datetime.datetime.today() > end_grace_period:
                    logger.warn("Deleting system_kjernen affiliation for person_id=%s,ou=%s,affi=%s last_date=%s,grace=%s" % (ent_id,ou,affi,last_date,cereconf.GRACEPERIOD_EMPLOYEE))
                    new_person.delete_affiliation(ou, affi, const.system_kjernen)
