...
fnr_exists = 0 # count for persons which we have colliding nins from FS
...
def process_person_callback(person_info):
    ...
    if person_info.has_key('aktiv'):
        for row in person_info['aktiv']:
            try:
                ...
                # Empty studieprogramkode-value
                errmsg = "Empty studieprogramkode-value"
    ...
    for dta_type in person_info.keys():
        ...
        if dta_type in ('fagperson',):
            ...
            for row in x:
                ...
                msg = "Adding affiliation 'student' at stedkode %s" % studieprog2sko[row['studieprogramkode']]
                #print msg
                logger.debug(msg)
    ...
    try:
        ...
        print "name: %s %s. Reason: %s" % (fornavn,etternavn,str(ie))
        fnr_exists += 1
        db.rollback()
        return
...
def usage():
    ...
    -e <filename>                   Filename for topics if not the default one
    --emne-file <filename>          Alias for -e)
    -g --generate-groups            
    -h --help                       Duh!
    -d --include-delete             Enable delete as well as import. For cleanup
    -v --verbose                    Add verbosity level
    -s --studieprogram-file         Filename for studyprograms if not the default one
    """ % sys.argv[0]
    sys.exit()

def main():
    global verbose, ou, logger, fnr2person_id, gen_groups, group
    global old_aff, include_delete, no_name, fnr_exists
    verbose = 0
    include_delete = False
    from Cerebrum import Constants
    foo = Constants.Constants(db)
    logger = Factory.get_logger("cronjob")
    opts, args = getopt.getopt(sys.argv[1:], 'vph:s:e:gdf', [
        'verbose', 'person-file=', 'help','studieprogram-file=',
    ...
    #for key,value in studieprog2sko.items():
    #    print key,":",value
    #for key,value in emne2sko.items():
    #    print key,":",value
    #sys.exit()
    ...
    print "Found %d persons without name." % no_name
    logger.info("Found %d persons with colliding national identity-numbers." % fnr_exists)
    print "Found %d persons with colliding national identity-numbers." % fnr_exists
