...
"""
This file is an NTNU-specific extension of Cerebrum.
...
def process_file(filename, db, const, person_old, person_new, exemptions):
    ...
    print "Source system is '%d' (%s)" % (source_system, source_system_name)

    for element in remove_cyclic_updates(parser):
        old, new = element["old"], element["new"]
        print "DEBUG: Process %s" % old 
        ...
        try:
            ...
            if source_system == const.system_fs:
                try:
                    person_old.find_by_external_id(id_type, old, const.system_bdb)
                except Errors.NotFoundError:
        ...
        try:
            ...
            if person_new.entity_id == person_old.entity_id:
                print "%s is registered with both %s (old) and %s (new)" % (person_new.entity_id, old, new)
                logger.info("%s is registered with both %s (old) and %s (new)",
                              person_new.entity_id, old, new)
            else:
