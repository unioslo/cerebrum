...
default_filename='hitos_employees.xml'
default_person_file=os.path.join(cereconf.DUMPDIR,'employees',default_filename)


class PagaDataParser(xml.sax.ContentHandler):
    """
    This class is used to iterate over all users in the HITOS FILE (same format as PAGA file). 
    """
    ...
    def startElement(self, name, attrs):
        if name == 'data':
            ...
            logger.error("unknown element: %s in hitos file" % name)
...
def get_existing_accounts():
    ...
    for row in person_obj.list_external_ids(id_type=const.externalid_fodselsnr, 
                                            source_system=const.system_hitos):
    ...
    for row in person.list_affiliations(source_system=const.system_hitos,
                                        fetchall=False):
    ...
    spread_list=[const.spread_uit_ldap_account, const.spread_uit_fd, const.spread_uit_ad_account]
...
def create_employee_account(fnr):
    ...
    if not owner:
        logger.error("Cannot create account to person %s, not from hitos" % fnr)
        return None
