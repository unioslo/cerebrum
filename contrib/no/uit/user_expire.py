...
def send_mail(uname, user_info, nr, forward=False):
    ...
    key = 'mail' + str(nr)
    try:
        mailfile = '/cerebrum/var/source/templates/' + cereconf.USER_EXPIRE_MAIL[key]
        f = open(mailfile)
    ...
    try:
        ...
        logger.debug("Sending %d. mail To: %s" % (nr, ', '.join(email_addrs)))
        Utils.sendmail(', '.join(email_addrs), email_from, subject, body.encode("iso8859-1"))
        return True
    except Exception,m:
        logger.error("Couldn't sent mail To: %s Reason: %s" %  (', '.join(email_addrs),m))
        return False
