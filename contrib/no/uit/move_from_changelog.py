            ...
            if current_change in allowed_changes:
                ...
                if entity_id not in account_expire:
                    try:
                        ...
                        # this dict
                        account_expire[entity_id] = True

                # check if account is expired
                if account_expire[entity_id] is False:
                ...
                if int(v['change_type_id']) in change_type_list:
                    if k not in deleted_entries:
                        logger.debug(
                            ...
                            "change entry:%s", current_change, k)
                        del changes[k]
                        deleted_entries.append(k)
                        continue
...
#
# collect changelog entries and delete entries according to the rules given
...
#
def collect_entries(from_date, to_date, out_file, db, only_expired,
                    change_types, dryrun):
    if out_file is not False:
        fh = open(out_file, "w")
    ...
    # collect all changelog entries from a single date
    changes = {}
    ...
    while modified_to_date <= to_obj:
        ...
        for item in retval:
            changes[item['change_id']] = {'tstamp': item['tstamp'],
                                          ...
                                          'change_program': item[
                                              'change_program']}
        filtered_changes = filter_entries(only_expired, change_types, changes,
                                          db)
...
def main(args=None):
    ...
    collect_entries(args.from_date, args.to_date, args.out_file, db,
                    args.only_expired, args.change_types, args.dryrun)
