...
#            Eks "internal:uit.no:fs:201:undenh:2004:vår"
...
def destroy_group(group_id, max_recurse):
    #print "max_recurse = %s" % max_recurse
    ...
    #print "destroying group %s" % group_id
...
class group_tree(object):
    ...
    def list_matches_1(self, *args, **kws):
	    ...
	    # the error-log.  
	    logger.debug("Ikke gyldig kull eller studieprog: args=%r, kws =%r, ret = %r" % (args[1],kws,ret))
            #logger.debug("kws = %r,ret = %r",kws,ret)
	    return () 
...
def prefetch_primaryusers():
    ...
    for a in account.list_accounts_by_type():
        ...
        #print "first person id = %s" % int(a['person_id'])
        #print "first a_id = %s" % int(a['account_id'])
    ...
    for row in person.list_external_ids(id_type=const.externalid_fodselsnr):
        #print "person_id = %s" % int(row['entity_id'])
...
def main():
    init_globals()
    dump_dir = os.path.join(cereconf.DUMPDIR, 'FS')
    #dump_dir = "/home/cerebrum/migrate/imports/"
