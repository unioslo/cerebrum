...
def _get_alternatives(account_name, ac):
    ac.clear()
...
def is_cnaddr_free(local_part, domain_part, db):
...
def get_cn_addr(username, try_all, uname2mailinfo, ac, db):
        ...
        for mailinfo in uname2mailinfo[username]:
            if mailinfo['domain'].endswith(dom_part):
                ...
                if mailinfo['local_part'].find('.') == -1:
                    ...
                    continue
                elif not is_cnaddr_free(mailinfo['local_part'], dom_part, db):
...
def save_ad_email(username, lp, dp, db):
    tmp_ac = Factory.get('Account')(db)
...
def process_mail(username, try_all, db, co, ac, ou, uname2mailinfo):
    tmp_ac = Factory.get('Account')(db)
    ...
    for uname in build:
        ...
        if check_affs(pers_affs=tmp_p_affs) or username:
            (lp, dp) = get_cn_addr(uname, try_all, uname2mailinfo, ac, db)
            if lp:
                logger.info("Uname will get '%s@%s'" % (lp, dp))
                save_ad_email(uname, lp, dp, db)
            else:
...
def main(inargs=None):
    ...
    process_mail(args.user, args.try_all, db, co, ac, ou, uname2mailinfo)
