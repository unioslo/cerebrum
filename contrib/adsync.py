...
def full_user_sync():
    ...
    for line in receive[1:-1]:
        ...
        if fields[3] in adusers:
            ...
            #
            # TBD: In this case two OUs should not have the same name,
            # better method would be to compare two lists.
            ...
            if ((full_name, account_disable, AD_HOME_DRIVE, home_dir,
                 login_script, AD_PASSWORD_EXPIRE)
                !=
                (fields[9], fields[17], fields[15], fields[7], fields[13],
                 fields[21])):
                sock.send('ALTRUSR&%s/%s&fn&%s&dis&%s&'
                          'hdir&%s&hdr&%s&ls&%s&pexp&%s&ccp&%s\n' % (
                    AD_DOMAIN, fields[3], full_name, account_disable,
                    home_dir, AD_HOME_DRIVE, login_script,
            ...
            if delete_users:
                ...
                if AD_LOST_AND_FOUND not in get_ad_ou(fields[1]):
                    ...
                    if sock.read() != ['210 OK']:
                        print 'Error moving:', fields[3], 'to', \
                              AD_LOST_AND_FOUND
    for user in adusers:
        ...
        if sock.read() == ['210 OK']:
            ...
            sock.send('ALTRUSR&%s/%s&pass&%s&fn&%s&dis&%s&hdir&%s&hdr&%s&'
                      'ls&%s&pexp&%s&ccp&%s\n' % (
                AD_DOMAIN, user, passw, full_name, account_disable, home_dir,
                AD_HOME_DRIVE, login_script, AD_PASSWORD_EXPIRE,
                AD_CANT_CHANGE_PW))
            sock.read()
        else:
...
## if an OU specified in Cerebrum is missing in AD.
## The correspondence of OU structure in AD and at UIO limit the OU tree in
## AD to one level from the root, until an OU module to AD is made."""
