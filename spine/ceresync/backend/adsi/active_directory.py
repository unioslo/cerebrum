...
#
# ADS_GROUP_TYPE_ constants
#
GLOBAL_GROUP = i32(0x00000002L)
DOMAIN_LOCAL_GROUP = i32(0x00000004L)
LOCAL_GROUP = i32(0x00000004L)
UNIVERSAL_GROUP = i32(0x00000008L)
SECURITY_ENABLED = i32(0x80000000L)
GROUP_TYPES = {
    GLOBAL_GROUP : "Global group",
    DOMAIN_LOCAL_GROUP : "Domain local group",
...
class AD_object(object):
    ...
    def __init__(self, obj=None, path=""):
        ...
        #
        if path:
            ##if not path.upper().startswith("LDAP://"):
            ##    path = "LDAP://" + path
        ...
        for name in self.properties:
            ...
            if value:
                if isinstance(name, unicode):
                    name = name.encode("cp1252", "ignore")
...
def find_user(name=None):
    ...
    for user in search("objectCategory='Person'", "sAMAccountName='%s' OR displayName='%s' OR cn='%s'" % (name, name, name)):
        return user

def find_computer(name=None):
    name = name or socket.gethostname()
    for computer in search("objectCategory='Computer'", "cn='%s'" % name):
        return computer
...
# arch-tag: bdf43c56-d825-11d9-8655-2481a842a0fe
