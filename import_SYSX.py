      ...
      if v:
         ...
         for aff in person.list_affiliations(int(ent_id),
            ...
            if mx.DateTime.today() > end_grace_period:
               logger.warn("Deleting sysX affiliation for person_id=%s,ou=%s,affi=%s last_date=%s,grace=%s" % (ent_id,ou,affi,last_date,grace))
...
def create_sysx_person(sxp):
    ...
    if (personnr != ""):
        try:
           ...
           logger.error("Ugyldig f�dselsnr: %s on sysX id=" % (personnr,id))
           return
    ...
    logger.info("Processing '%s', SysXId=%s, fnr='%s'" % (fullt_navn,id,personnr))
    if fnr_found:
        person=pers_fnr
    else:
        person=pers_sysx
...
def main():
    ...
    fmt_str="Stats: Added: %d, Updated=%d, Skipped=%d, Unchanged=%d, Deleted affs=%d"
