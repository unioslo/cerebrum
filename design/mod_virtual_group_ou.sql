...
(
    ...
    affiliation         NUMERIC(6,0) NULL REFERENCES person_affiliation_code,
    affiliation_source  NUMERIC(6,0) NULL REFERENCES authoritative_system_code,
    affiliation_status  NUMERIC(6,0) NULL REFERENCES person_aff_status_code(status),

    /* how recursion is applied */
    recursion           NUMERIC(6,0) NOT NULL REFERENCES virtual_group_ou_recursion_code,
...
category:main;
CREATE INDEX virtual_group_ou_aff_index ON virtual_group_ou(ou_id, recursion, affiliation);
category:main;
CREATE INDEX virtual_group_ou_total_index ON virtual_group_ou(ou_id,
    affiliation, affiliation_source, affiliation_status, recursion,
    ou_perspective, member_type);
category:main;
CREATE INDEX virtual_group_ou_nopers_index ON virtual_group_ou(ou_id,
    affiliation, affiliation_source, affiliation_status, recursion,
    member_type);
category:main;
CREATE INDEX virtual_group_ou_nosrc_index ON virtual_group_ou(ou_id,
    affiliation, recursion, ou_perspective, member_type);
