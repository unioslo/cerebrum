...
#
# if keep_last == True => remove last change_type for each subject from the list
# if only_expired == true => remove entries belonging to non-epxired accpunts
# if change_types != False => remove all change_types not in this list
# 
    ...
    for k,v in changes.items():
        ...
        if only_expired == True:
            ...
            if current_change in allowed_changes:
                ...
                if entity_id not in account_expire:
                    try:
                        ...
                        logger.debug("adding account_expire[%s] = %s" % (entity_id,account_expire[entity_id]))
                    except Errors.NotFoundError:
                        logger.debug("unable to find account entity_id:%s, but we know its an account entry. Delete change entry:%s" % (entity_id,k))
                        account_expire[entity_id] = True # add account to account_expire dict. this will speedup the search when more and more accounts are found in this dict
                      
                    
                # check if account is expired
                if account_expire[entity_id] == False:
                    # account is not expired. remove from list (that means do not delete changelog entries for this account)
                    if k not in deleted_entries:
                        logger.debug("account:%s is NOT expired. Do not delete change entry:%s " % (v['subject_entity'],k))
                        del changes[k]
                ...
                if k not in deleted_entries:
                    logger.debug("This is not an account. Do not delete change entry:%s" % k)
                    del changes[k]
                    deleted_entries.append(k)
                    continue
    
            if change_types != False:
                #logger.debug("look for change types:%s" % change_type_list)
                #print "compare:%s against: %s" % (int(v['change_type_id'],change_type_list))
                if int(v['change_type_id']) in change_type_list:
                    if k not in deleted_entries:
                        logger.debug("change entry is of valid type:%s. Do not delete change entry:%s" % (current_change,k))
                        del changes[k]
                        deleted_entries.append(k)
                        continue
            ...
            if change_types != False:
                ...
                if int(v['change_type_id'])  in change_type_list:
                    if k not in deleted_entries:
                        logger.debug("change type:%s is in list of valid change types. Do not delete change entry:%s" % (v['change_type_id'],k))
                        del changes[k]
                        deleted_entries.append(k)
                        continue
...
#
def collect_entries(from_date,to_date,out_file,db,keep_last,only_expired,change_types,dryrun):
    if out_file != False:
        fh = open(out_file,"w")
    ...
    while modified_to_date <= to_obj:
        logger.debug("processing day: %s - %s" %(modified_from_date,modified_to_date))
        retval = db.get_log_events_date(sdate=modified_from_date,edate=modified_to_date)
        modified_from_date = modified_from_date + timedelta(days=1)
        modified_to_date = modified_to_date + timedelta(days=1)
        for item in retval:
            changes[item['change_id']] = {'tstamp' : item['tstamp'], 'subject_entity' : item['subject_entity'], 'change_type_id' : item['change_type_id'],'dest_entity' : item['dest_entity'],'change_params' : item['change_params'],'change_by' : item['change_by'],'change_program' : item['change_program']}
            #print "date: %s, change_id:%s, subject:%s, type:%s" % (item['tstamp'],item['change_id'],item['subject_entity'],item['change_type_id'])
        filtered_changes = filter_entries(keep_last,only_expired, change_types,changes,db)
        ...
        if dryrun == False:
            if changes_since_last_commit > 1000000:
                ...
                logger.debug("commit after: %s changes" % changes_since_last_commit)
                changes_since_last_commit = 0
...
def main(args=None):
    ...
    parser.add_argument('--from',
                        ...
                        dest='from_date',
                        action='store',
                        help='remove entries from this date. Format is YYYY-MM-DD')
    parser.add_argument('--to',
                        required=True,
                        action='store',
                        dest='to_date',
                        help='Remove entries to this date. Format is YYYY-MM-DD')
    ...
    parser.add_argument('--keep-last',
                        ...
                        action='store_true',
                        default=False,
                        help='Do not delete last entry of each change type (NOT WORKING YET)')
    parser.add_argument('--only-expired-accounts',
                        dest='only_expired',
                        action='store_true',
                        default=False,
                        help='Only delete change entries belonging to expired accounts')
    parser.add_argument('--change_types=',
                        dest='change_types',
                        action='store',
                        default=False,
                        help='Only delete change entries listed in change_types attribute. format is comma separated list with values as listed in change_type.change_type_id in the database')
    ...
    db = get_db()
    collect_entries(args.from_date,args.to_date,args.out_file,db,args.keep_last,args.only_expired,args.change_types,args.dryrun)
