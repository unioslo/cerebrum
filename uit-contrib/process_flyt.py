...
def get_existing_accounts(person_type):
    ...
    for row in person.list_affiliations(source_system=const.system_flyt, fetchall=False):
    ...
    for row in account_obj.search(expire_start=None):
        ...
        if(account_name.endswith(cereconf.USERNAME_POSTFIX['sito'])):
            ...
            logger.debug("%s is a sito account. Do not process as part of flyt import" % (account_name))
            continue
    ...
    if(person_type =='E'):
        ...
        spread_list = [int(const.Spread(x)) for x in cereconf.FLYT_EMPLOYEE_SPREADLIST]
    elif(person_type == 'S'):
        # load student spreadlists
        logger.info("Loading spreads... %s " % cereconf.FLYT_STUDENT_SPREADLIST)
        spread_list = [int(const.Spread(x)) for x in cereconf.FLYT_STUDENT_SPREADLIST]
...
class Build:
         ...
         for person in fh:
             ...
             if(person[0] == "#"):
                 ...
                 try:
                     ...
                     logger.warn("person:%s is missing required keys. Processing stopped for this person" % pers_dict )
                     continue
                 # only collect persons with eduPersonAffiliation equal to faculty or staff
                 if ((('faculty' in affiliation_type) or ('staff' in affiliation_type)) and (self.type =='E')):
     ...
     def _calculate_spreads(self,acc_affs,new_affs):
         ...
         if(self.type == 'E'):
             default_spreads= [int(const.Spread(x)) for x in cereconf.FLYT_EMPLOYEE_SPREADLIST]
         elif(self.type == 'S'):
             default_spreads= [int(const.Spread(x)) for x in cereconf.FLYT_STUDENT_SPREADLIST]
     ...
     def notify_user(self,email,username):
         if(os.path.isfile(self.email_template) != True):
             ...
             foo = Utils.sendmail(email,email_from,email_subject,email_body.decode('utf-8').encode("iso-8859-1"),cc,debug=False)
     ...
     def process_person(self, fnr, expire_date):
        ...
        #pprint(persons[int(fnr)])
         ...
         if not p_obj:
             logger.warn("Unknown person %s. Processing stopped for this person" % (fnr,))
             return None
        ...
        # check if person has an account
         ...
         if new_account == False:
             current_expire = mx.DateTime.Date(int(current_expire_string[0:4]),int(current_expire_string[5:7]),int(current_expire_string[8:10]))
         else:
	 ...
	 #	              update_expire_date(but not to a date after agreement_expire_date) 
         ...
         if(today > current_expire):
             ...
             mx_flyt_expire = mx.DateTime.Date(int(flyt_expire[0:4]),int(flyt_expire[5:7]),int(flyt_expire[8:10]))
             if(mx_flyt_expire > today):
                 # OK
                 new_expire = str(get_expire_date(flyt_expire))
                 logger.debug("new expire date from flyt data. update expire date to:%s" % new_expire)
             else:
                 #
             ...
             if new_account == True:
                 # OK
                 logger.debug("use account OBJECT exire date:%s" % acc_obj.get_expire_date())
                 new_expire = acc_obj.get_expire_date()
             else:
                 # OK
                 logger.debug("use account in DB expire_date:%s" % current_expire)
                 new_expire = current_expire
         ...
         if p_obj.get_deceased_date() is not None:
             ...
             if current_expire != new_expire:
                 logger.warn("Account owner deceased: %s. Processing stopped for this person" % (acc_obj.get_uname()))
                 new_deceased = True
        ...
        # Har ikke personnavn tilgjengelig pr nuh..
         ...
         if p_obj.get_affiliations():
             ...
             if to_add:
                 logger.info("change in spread. add following spreads:%s" % to_add)
...
def create_flyt_account(fnr,expire_date):
    logger.debug("creating account for person:%s. flyt wants expire date to be:%s" % (fnr,expire_date))
    owner=persons.get(fnr)
    if not owner:
        logger.error("Cannot create account to person %s, not from flyt. Processing stopped for this person" % fnr)
        return None
...
def get_expire_date(flyt_expire_date=None):
    if flyt_expire_date == None:
        ...
        #
        cerebrum_expire = mx.DateTime.now()+mx.DateTime.RelativeDateTime(years=1)
        ...
        cerebrum_expire = mx.DateTime.Date(int(flyt_expire_date[0:4]),int(flyt_expire_date[5:7]),int(flyt_expire_date[8:10]))
    ...
    if cerebrum_expire > ff_start and cerebrum_expire < ff_slutt:
        print "1. expire date is set to:%s" % mx.DateTime.DateTime(today.year,9,1)
...
def main():
    ...
    global accounts
    try:
        opts,args=getopt.getopt(sys.argv[1:],'E:t:dp:ehT:',['email_template=','type=','dryrun','person_file=','email','help','time_to_live'])
    ...
    for opt,val in opts:
        ...
        if opt in ('-e','--email'):
            email_forward = True
            logger.info('will set email forward to email address from external organization')
        if opt in ('-h','--help'):
            usage()
