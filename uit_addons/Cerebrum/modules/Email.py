...
class EmailAddress(Entity_class):
    ...
    def write_db(self):
        ...
        is_new = not self.__in_db
        if is_new:
    # NB!! kbj005, 201708265: Have added create_date to the insert below here as it is needed.
            self.execute("""
            INSERT INTO [:table schema=cerebrum name=email_address]
              (address_id, local_part, domain_id, target_id,
               change_date, expire_date, create_date)
            VALUES (:a_id, :lp, :d_id, :t_id, [:now], :expire, [:now])""",
                         {'a_id': self.entity_id,
