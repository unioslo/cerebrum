...
def get_existing_accounts():
        ...
        if tmp is not None:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("storing stedkode: %s for sys_x_user:%s" % ou_stedkode_mapping[row['ou_id']],tmp)
                except:
                    logger.info("sysx-id:%s has expired stedkode:%s." %(row['person_id'],row['ou_id']))
    ...
    for row in account_obj.list(filter_expired=False,fetchall=False):
        ...
        if (account_name_obj.account_name.endswith(cereconf.USERNAME_POSTFIX['sito'])):
            #if account_name_obj.account_name[-2:] == cereconf.USERNAME_POSTFIX['sito']:
            logger.debug("account:%s is a sito account and removed from systemx account process list" % (account_name_obj.account_name))            
            continue

        #
        # Exclude accounts that ends with 999 (they are admin accounts and not to be counted
        # when checking for existing accounts)
        #
        if account_name_obj.account_name[3:6] =='999':
            logger.info("account_name:%s is an admin account and excluded from list" % account_name_obj.account_name)
            continue
    ...
    for acc_id, tmp in tmp_ac.items():
        ...
        logger.info("appending sys-x id:%s, account id:%s to list of existing accounts" % (sysx_id,acc_id))
...
def send_mail(type, person_info, account_id):
    ...
    # Deaktivert 2010-08-30 etter e-post fra Ranveig 13. august 
    #if type=='ansvarlig':
...
class Build(object):
    ...
    def _process_sysx(self,sysx_id, person_info):
        ...
        # today = current date
        if ((new_expire > today) and (new_expire > current_expire) and (current_expire < today) and (user_mail_message == False)):
                mailq.append( {
                    'account_id': acc_id,
                    'person_info': person_info, 
                    'template': 'bruker'
        ...
        for skofilter in cereconf.EMPLOYEE_FILTER_EXCHANGE_SKO:
            if skofilter == person_sko[0:len(skofilter)] and not could_have_exchange:
        ...
        if changes:
            ...
            sko.clear()
            try:
                sko.find_stedkode(int(person_info['ou'][0:2]), int(person_info['ou'][2:4]), int(person_info['ou'][4:6]), cereconf.DEFAULT_INSTITUSJONSNR)
